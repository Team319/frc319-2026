...
public class RightBumpToOutpost {
  ...
  public static Command getCommand(Drive drive) {
    ...
    return 
    Commands.sequence(
      Commands.runOnce(() -> System.out.println("Running Right Bump to Outpost Auto")),
      Commands.runOnce(() -> drive.setPose(AllianceFlipUtil.apply(new Pose2d(new Translation2d(3.5, 2.5), new Rotation2d(Math.toRadians(defaultHeading)))))),
      Commands.waitSeconds(0.5),
      
      Commands.parallel(
        DriveCommands.followPathCommand("go_right_bump",false),
